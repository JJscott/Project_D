...
// first run needs special handling:
// deltaS tables dont include phase function factor after single scatter,
// but they do during multiple scattering. the phase function factor is divided
// out by copy_inscatter_multiple and re-applied at runtime
uniform bool first;
...
void inscatter(float r, float mu, float muS, float nu, out vec3 raymie) {
	...
	for (int itheta = 0; itheta < INSCATTER_SPHERICAL_INTEGRAL_SAMPLES; ++itheta) {
		...
		for (int iphi = 0; iphi < 2 * INSCATTER_SPHERICAL_INTEGRAL_SAMPLES; ++iphi) {
			...
			// see Eq (7)
			raymie += raymie1 * (betaR * exp(-(r - Rg) / HR) * pr2 + betaM * exp(-(r - Rg) / HM) * pm2) * dw;
		}
	}
