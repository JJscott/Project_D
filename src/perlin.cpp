...
perlin::perlin(){
	m_random = initial3d::random();
	//InitGradients
	for (int i = 0; i < GRADIENT_TABLE_SIZE; i++){
		double z = 1.0 - 2.0 * m_random.nextDouble();
		double r = sqrt(1.0 - z * z);
		double theta = 2 * pi() * m_random.nextDouble();
...
perlin::perlin(long seed){
	m_random = initial3d::random(seed);
	//InitGradients
	for (int i = 0; i < GRADIENT_TABLE_SIZE; i++){
		double z = 1.0 - 2.0 * m_random.nextDouble();
		double r = sqrt(1.0 - z * z);
...
double perlin::getNoise(double x, double y, double z, int octaves){
	double noise = 0;
...
double perlin::getNoise(double x, double y, double z){
	int ix = (int)floor(x);
	double fx0 = x - ix;
	double fx1 = fx0 - 1;
...
double perlin::smooth(double x){
	return x * x * (3 - 2 * x);
}

double perlin::lattice(int ix, int iy, int iz, double fx, double fy, double fz){
	int index = getIndex(ix, iy, iz);
	int g = index * 3;
	return m_gradients[g] * fx + m_gradients[g + 1] * fy + m_gradients[g + 2] * fz;
}

int perlin::getIndex(int ix, int iy, int iz){
	return permutate(ix + permutate(iy + permutate(iz)));
}

int perlin::permutate(int x){
	int mask = GRADIENT_TABLE_SIZE - 1;
	return m_perm[x & mask];
}

double perlin::lerp(double t, double value0, double value1){
